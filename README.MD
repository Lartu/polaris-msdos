...
- If you are on a **MS-DOS** system, get [Turbo C](https://edn.embarcadero.com/article/20841). Then from Turbo C open `polaris.c` and then go to Compile 🡒 Make EXE file. If you get an error stating that the project consists of only one line change all line breaks in polaris.c (\n) to carriage returns + line breaks (\r\n).
